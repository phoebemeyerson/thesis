...
data %>% group_by(SUBSECTION, rep) %>% count() %>%
  ungroup() %>%
  group_by(SUBSECTION) %>%
...
fire_domains <- unique(fire_true_est$SUBSECTION)
...
fire_feature_ids <- c("M333Ag", "M333Ab", "M333Aa", "M333Ba", "M333Cb", "M333Cc","M333Ca", "M333Cf", "M333Ai", "M333Da", "M333Ac", "M333Cg", "M333Ce", "M333Db", "M333Dd", "M333Ch", "M333De", "THORNEFIRE", "M333Dc", "WHITFIRE", "M333Ah", "CARPFIRE", "M333Ad", "M333Ae", "CHIPFIRE", "M333Bb", "M333Bc")
...
for (i in 1:nreps) {
  ...
  fire_results <- rbind(fire_results, get_poststrat(rep_split, np, fire_prop_split,
                                          fire_domains, i, fire_true_est$true_val))
...
saveRDS(fire_results, "fire_results1000.rds")
```
...
```{r}
# METRICS 
fire_results <- readRDS("fire_results1000.rds") %>%
  mutate(method = fct_recode(method, "SSMA" = "SMA"))
...
# SHOW TO KELLY
fire_metrics %>% filter(region == "THORNEFIRE") %>%
  select(method, prb, cov_rate, emp_mse, prb_mse)

fire_metrics %>% filter(region == "CARPFIRE") %>%
  select(method, prb, cov_rate, emp_mse, prb_mse)

fire_metrics %>% filter(region == "CHIPFIRE") %>%
  select(method, prb, cov_rate, emp_mse, prb_mse)

fire_metrics %>% filter(region == "WHITFIRE") %>%
  select(method, prb, cov_rate, emp_mse, prb_mse)


res_thorne <- fire_results %>% 
  filter(region == "THORNEFIRE") %>%
  group_by(method) %>%
  mutate(emp_mean = mean(est),
         method = factor(method, levels = c("HT", "PS", "GREG", "SSMA"))) 
res_carp <- fire_results %>% 
  filter(region == "CARPFIRE") %>%
  group_by(method) %>%
  mutate(emp_mean = mean(est),
         method = factor(method, levels = c("HT", "PS", "GREG", "SSMA"))) 
res_chip <- fire_results %>% 
  filter(region == "CHIPFIRE") %>%
  group_by(method) %>%
  mutate(emp_mean = mean(est),
         method = factor(method, levels = c("HT", "PS", "GREG", "SSMA"))) 
res_whit <- fire_results %>% 
  filter(region == "WHITFIRE") %>%
  group_by(method) %>%
  mutate(emp_mean = mean(est),
         method = factor(method, levels = c("HT", "PS", "GREG", "SSMA"))) 

# SHOW TO KELLY
fire_est_boxplot_thorne <- ggplot(res_thorne, aes(x = method, y = est)) +
  geom_boxplot() +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "THORNEFIRE"))$true_val, 
             col = "firebrick", lwd = 1.2) +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "M333Dc"))$true_val, 
             col = "darkolivegreen", lwd = 1.2, linetype = "dashed") +
  labs(x = "Method", y = "Dry Biomass Estimates (Tons)") +
  theme_minimal(base_size = 24)

fire_est_boxplot_carp <- ggplot(res_carp, aes(x = method, y = est)) +
  geom_boxplot() +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "CARPFIRE"))$true_val, 
             col = "firebrick", lwd = 1.2) +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "M333Ad"))$true_val, 
             col = "darkolivegreen4", lwd = 1.2, linetype = "dashed") +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "M333Ae"))$true_val, 
             col = "darkolivegreen", lwd = 1.2, linetype = "dashed") +
  labs(x = "Method", y = "Dry Biomass Estimates (Tons)") +
  theme_minimal(base_size = 24)

fire_est_boxplot_chip <- ggplot(res_chip, aes(x = method, y = est)) +
  geom_boxplot() +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "CHIPFIRE"))$true_val, 
             col = "firebrick", lwd = 1.2) +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "M333Bb"))$true_val, 
             col = "darkolivegreen", lwd = 1.2, linetype = "dashed") +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "M333Bc"))$true_val, 
             col = "darkolivegreen4", lwd = 1.2, linetype = "dashed") +
  labs(x = "Method", y = "Dry Biomass Estimates (Tons)") +
  theme_minimal(base_size = 24)

fire_est_boxplot_whit <- ggplot(res_whit, aes(x = method, y = est)) +
  geom_boxplot() +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "WHITFIRE"))$true_val, 
             col = "firebrick", lwd = 1.2) +
  geom_hline(yintercept = (fire_true_est %>% 
                             filter(SUBSECTION == "M333Ah"))$true_val, 
             col = "darkolivegreen", lwd = 1.2, linetype = "dashed") +
  labs(x = "Method", y = "Dry Biomass Estimates (Tons)") +
  theme_minimal(base_size = 24)

ggsave("thorne_ests.png", fire_est_boxplot_thorne)
ggsave("whit_ests.png", fire_est_boxplot_whit)
ggsave("carp_ests.png", fire_est_boxplot_carp)
ggsave("chip_ests.png", fire_est_boxplot_chip)

```

```{r}
m333_col <- m333_fire %>%
  mutate(color = case_when((SUBSECTION == "CARPFIRE" |
                             SUBSECTION == "CHIPFIRE" |
                             SUBSECTION == "WHITFIRE" |
                             SUBSECTION == "THORNEFIRE") ~ "fire",
                           (SUBSECTION == "M333Dc" |
                              SUBSECTION == "M333Ah" |
                              SUBSECTION == "M333Bb" |
                              SUBSECTION == "M333Ae") ~ "neighbA",
                           (SUBSECTION == "M333Bc" |
                              SUBSECTION == "M333Ad") ~ "neighbB",
                           TRUE ~ "nothing"))
fire_map <- ggplot() +
  geom_sf(data = m333_col, aes(fill = factor(color))) +
  scale_fill_manual(values = setNames(c("firebrick2", "darkolivegreen", "darkolivegreen4", "darkolivegreen3"), c("fire", "neighbA", "neighbB", "nothing"))) +
  guides(fill = FALSE) +
  theme_minimal(base_size = 14) +
  theme_void()
  
ggsave("fire_m333.png", fire_map)
...
count_fire <- data %>%
  filter(SUBSECTION == "M333AhFIRE" | 
           SUBSECTION == "M333DaFIRE" | 
           SUBSECTION == "M333DcFIRE") %>%
  group_by(SUBSECTION, rep) %>%
  count()

hist((count_fire %>% filter(SUBSECTION == "M333AhFIRE"))$n, breaks = 6)
length((count_fire %>% filter(SUBSECTION == "M333AhFIRE"))$n)
hist((count_fire %>% filter(SUBSECTION == "M333DaFIRE"))$n, breaks = 5)
length((count_fire %>% filter(SUBSECTION == "M333DaFIRE"))$n)
hist((count_fire %>% filter(SUBSECTION == "M333DcFIRE"))$n)
length((count_fire %>% filter(SUBSECTION == "M333DcFIRE"))$n)

Da_ones <- (count_fire %>% filter(SUBSECTION == "M333DaFIRE" & n == 1))$rep

Da_twoplus <- fire_res725 %>% filter(region == "M333DaFIRE") %>% filter(!(rep %in% Da_ones))

Da_twoplus_metrics <- Da_twoplus %>%
  group_by(method) %>%
  mutate(cov_rate = mean(ifelse(true_val >= lower & true_val <= upper, 1, 0)),
         emp_mean = mean(est),
         emp_var = mean((est - emp_mean)^2),
